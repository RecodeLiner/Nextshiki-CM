...
data class PeopleObject(
    ...
    @SerialName("groupped_roles") val grouppedRoles: List<JsonElement> = emptyList(),
    @SerialName("roles") val roles: List<Roles> = emptyList(),
    @SerialName("works") override val works: List<Works> = emptyList(),
...
) : CommonSearchInterface, ISearchParticipation {
    override val searchType: SearchType
        get() = SearchType.People
}
